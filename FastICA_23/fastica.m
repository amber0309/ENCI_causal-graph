...
% Copyright (c) Jarmo Hurri, Hugo Gävert, Jaakko Särel? and Aapo Hyvärinen.
%
...
[mixedsig, mixedmean] = remmean(mixedsig); %mixedsig(after remove mean) = size(original mixedsig); mixedmean(N*1), element (ni,1) is the mean of row ni of vector

[Dim, NumOfSampl] = size(mixedsig); % dim = row number, NumOfSampl = column number
...
if only > 1
  ...
  if jumpWhitening == 3,
    ...
    % whitesig is still row - attri, col - sample
  end
